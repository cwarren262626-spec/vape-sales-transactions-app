...
async function handleSaveTransaction(e) {
  ...
  const { error: insertError } = await supabase.from("SalesTransactions").insert({
...
function openProductDialog(existing) {
  ...
  $("#pd-save", backdrop).addEventListener("click", async () => {
    ...
    const query = isNew
      ? supabase.from("ProductList").insert(payload)
      : supabase.from("ProductList").update(payload).eq("id", existing.id);
...
function openDeleteDialog(product) {
  ...
  $("#del-confirm", backdrop).addEventListener("click", async () => {
    const { error } = await supabase.from("ProductList").delete().eq("id", product.id);
